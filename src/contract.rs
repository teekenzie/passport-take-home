...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::SetScore {
            ...
            entering_token,
            score,
        } => try_set_score(deps, info, entering_addr, entering_token, score),
    }
}
...
pub fn try_set_score(
    ...
    let update_score = |_| -> StdResult<i32> { Ok(score) };
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetScore {
            entering_addr,
            entering_token,
        } => to_binary(&query_score(deps, entering_addr, entering_token)?),
    }
}
...
mod tests {
    ...
    fn set_score() {
        ...
        let msg = ExecuteMsg::SetScore {
            entering_addr: "anyone".to_string(),
            entering_token: "token Mirror".to_string(),
            score: 3,
        };
        ...
        let auth_info = mock_info("creator", &coins(2, "token"));
        let msg = ExecuteMsg::SetScore {
            entering_addr: "anyone2".to_string(),
            entering_token: "token Mirror".to_string(),
            score: 13,
        };
        let _res = execute(deps.as_mut(), mock_env(), auth_info, msg).unwrap();
        ...
        let msg = ExecuteMsg::SetScore {
            entering_addr: "anyone".to_string(),
            entering_token: "token TerraUST".to_string(),
            score: 57,
        };
        let _res = execute(deps.as_mut(), mock_env(), auth_info, msg).unwrap();
